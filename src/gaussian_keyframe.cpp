...
void GaussianKeyframe::setCameraParams(const Camera& camera)
{
    ...
    case 3: // Longitude-latitude
    {
        float focal_length_x = image_width_ / 3.464f; // 120 degrees perspective FoVx by default
        float focal_length_y = image_height_ / 1.155f; // 60 degrees perspective FoVy by default
        this->FoVx_ = graphics_utils::focal2fov(focal_length_x, camera.width_);
        this->FoVy_ = graphics_utils::focal2fov(focal_length_y, camera.height_);
        this->set_camera_ = true;
    }
...
void GaussianKeyframe::computeTransformTensors()
{
    if (this->set_pose_ && this->set_camera_) {
        Eigen::Matrix4f Tcw;
        Tcw = this->getWorld2View2(this->trans_, this->scale_);
        this->world_view_transform_ = tensor_utils::EigenMatrix2TorchTensor(
            Tcw,
            torch::kCUDA
        ).transpose(0, 1);

        Eigen::Matrix3f Rwc = Tcw.topLeftCorner<3, 3>().inverse();
        this->RwcT_ = tensor_utils::EigenMatrix2TorchTensor(
            Rwc,
            torch::kCUDA
        ).transpose(0, 1);
