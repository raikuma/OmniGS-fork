...
GaussianRasterizerFunction::forward(
    ...
    torch::Tensor means2D, /*输入是全零，只是为了和backward对应，让torch输出对2D screen-space坐标(值域是-1到1)的导数*/
    torch::Tensor sh,
    ...
    GaussianRasterizationSettings raster_settings)
{
    ...
    auto rasterization_result = RasterizeGaussiansCUDA(
        ...
        (int)raster_settings.camera_type_,
        raster_settings.render_depth_
    );
    ...
    ctx->saved_data["camera_type"] = (int)raster_settings.camera_type_;
    ...
    torch::autograd::tensor_list grad_outputs)
{
    ...
    auto camera_type = ctx->saved_data["camera_type"].toInt();
    ...
    torch::Tensor cov3D_precomp)
{
    ...
    torch::autograd::tensor_list result;
    result = rasterizeGaussians(
        means3D,
        means2D,
        shs,
        colors_precomp,
