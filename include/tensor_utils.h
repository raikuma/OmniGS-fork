...
inline Eigen::Matrix4f TorchTensor2EigenMatrix4x4(
    torch::Tensor tensor)
{
    assert(tensor.sizes().size() == 2 && tensor.sizes()[0] == 4 && tensor.sizes()[0] == 4);

    Eigen::Matrix4f eigen_matrix_T(tensor.detach().contiguous().cpu().data_ptr<float>());
    Eigen::Matrix4f eigen_matrix = eigen_matrix_T;
