...
public:
    ...
    torch::Tensor world_view_transform_;    ///< transform tensors， Tcw^T
    torch::Tensor projection_matrix_;       ///< transform tensors, Proj^T, camera space[tx,ty,tz], screen space [sx,sy,sz,sw] = [cot(fovx/2)*tx, cot(fovy/2)*ty, (tz-zn)*zf/(zf-zn), tz]
    torch::Tensor RwcT_;                    ///< transform tensors， Rwc^T
    torch::Tensor full_proj_transform_;     ///< transform tensors, (Proj @ Tcw)^T
